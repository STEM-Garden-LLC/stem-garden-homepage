...
const cellsInLine = getCellsInLinesObject()
const numberOfLines = Object.keys(cellsInLine).length
...
try {
  const cellsInLine = getCellsInLinesObject()
  const linesThatIncludeCell = getLinesThatCellIsMemberOfObject(cellsInLine)

  fs.writeFileSync('/Users/nigelwilson/Desktop/connect_four/cells_in_line.json', JSON.stringify(cellsInLine))
  fs.writeFileSync('/Users/nigelwilson/Desktop/connect_four/lines_that_include_cell.json', JSON.stringify(linesThatIncludeCell))
...
function getCellsInLinesObject() {
  let object = {}
  let currentLineId = 0
...
function getLinesThatCellIsMemberOfObject(cellsInLine) {
  let object = {}
  for (let c = 0; c < totalCells; c++) {
    object[c] = []
  }
  for (let lineId = 0; lineId < numberOfLines; lineId++) {
    let cells = Object(cellsInLine)[lineId.toString()]
    // console.log(`Looking at line ${lineId} found cells in line to be: [${cells}]`);
    cells.forEach(cell => {
      object[cell] = object[cell].concat(lineId)
    })
  }
...
function getRowByCellId(cellId) {
  return (Math.floor(cellId / cellsPerRow))
}
function getColByCellId(cellId) {
  return (cellId % cellsPerRow)
}
